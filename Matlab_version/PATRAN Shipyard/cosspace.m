function x = cosspace(startPoint, endPoint, numPoints, varargin)
...
angleInc = (2*startAngle)/(numPoints-1);
...
curAngle = -startAngle + angleInc;
for idx = 2:ceil(numPoints/2)
    x(idx)=startPoint+midPoint+(1/sin(startAngle))*midPoint*sin(curAngle);
