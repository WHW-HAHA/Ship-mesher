...
% Last Modified by GUIDE v2.5 15-Sep-2015 13:15:26
...
function meshgui_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    handles.ship = evalin('base','ship');
    handles.file = evalin('base','file');
    handles.sf = evalin('base','sf');
    set(handles.edit_kxx,'String',handles.ship.B*0.25);
    set(handles.edit_kyy,'String',handles.ship.L*0.3);
...
function pushbutton_compute_Callback(hObject, eventdata, handles)
    ...
    info = compute_panelsize(handles.file,handles.sf,kxx,kyy,KG,str2double(get(handles.edit_waterdepth,'String')));
    set(handles.text_info,'String','Please check if figures and output are reasonable.')
    handles.t = table(['Lpp  ';'B    ';'D    ';'T    ';'Disp.';'wn_3 '],[ship.L;ship.B;ship.D;ship.D-handles.sf.reduc;info.displacement;info.wn_3],...
	...
	guidata(hObject,handles);
    set(handles.text_estimated,'String',info.nowpsize);
    set(handles.text_recommended,'String',info.needpsize);
    set(handles.checkbox_splitpanels,'Enable','on');
...
function pushbutton_mesh_Callback(hObject, eventdata, handles)
    remesh_patranfile(handles.file,handles.sf,get(handles.checkbox_splitpanels,'Value'))
    [pathstr, name] = fileparts(handles.file); 
    set(handles.text_info,'String',['Patran file saved as ' name '_scaled.pat'])
...
function pushbutton_file_Callback(hObject, eventdata, handles)
    ...
    set(handles.text_info,'String',['File exported as ' name '_info.csv'])
...
% --- Executes on button press in checkbox_splitpanels.
function checkbox_splitpanels_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_splitpanels (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_splitpanels
